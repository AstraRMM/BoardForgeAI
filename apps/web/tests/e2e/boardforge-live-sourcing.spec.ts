...
test('live supplier sourcing UI shows DigiKey and Mouser evidence without fake stock', async ({ page }) => {
  ...
  await expect(page.getByText(/Authenticated Through Local Engine/i)).toBeVisible()
  await expect(page.getByText(/Digikey Productinformationv4 Reachable/i)).toBeVisible()
  await expect(page.getByText(/Mouser Search Api Verified/i)).toBeVisible()
  ...
  await expect(page.getByText(/Mouser Out Of Stock Digikey In Stock/i)).toBeVisible()
