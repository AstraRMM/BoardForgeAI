    ...
    <section className="grid gap-3 md:grid-cols-2">
      {projects.map((project) => (
        <div key={project} className="rounded-lg border border-slate-800 bg-slate-900 p-4">
          ...
          <p className="mt-2 text-sm text-slate-400">Generates local preview, review, risk, routeability, and download evidence. Sourcing remains NOT_CHECKED without API keys.</p>
        </div>
      ))}
    </section>
  )
