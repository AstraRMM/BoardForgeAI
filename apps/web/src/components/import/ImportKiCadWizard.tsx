...
export function ImportKiCadWizard() {
  ...
  return <section className="bf-workspace-panel" aria-labelledby="import-launch-state">
    ...
    <div className="bf-button-row"><button type="button" className="bf-new-board-primary" onClick={saveImport} disabled={!files.length || saved}>{saved ? 'Saved to Projects' : 'Save browser project'}</button>{savedProjectId && <Link href={`/projects/${encodeURIComponent(savedProjectId)}`} className="bf-new-board-secondary">Open saved project</Link>}<Link href="/projects" className="bf-new-board-secondary">All projects</Link><Link href="/settings/plugin" className="bf-new-board-secondary">Pair for KiCad validation</Link></div>
