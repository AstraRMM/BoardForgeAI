...
export function BrowserPcbEditor(){
 ...
 const choose=(id:Id,e:React.MouseEvent)=>{e.stopPropagation();setSelected(s=>{const n=new Set(e.shiftKey?s:[]);n.has(id)?n.delete(id):n.add(id);return n})}
 ...
 useEffect(()=>{const key=(e:KeyboardEvent)=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='z'){e.preventDefault();e.shiftKey?redo():undo()}if(e.key==='Delete'||e.key==='Backspace'){if(selected.size){commit(deleteSelection(board,selected));setSelected(new Set())}}};window.addEventListener('keydown',key);return()=>window.removeEventListener('keydown',key)})
 ...
 return <div className={styles.shell}>
  <header><strong>Board workspace</strong><span>{board.title}</span><span className={styles.saved}>Local model · KiCad-compatible</span></header>
  ...
  <aside className={styles.layers}><h2>Layers</h2>{LAYERS.map(l=><label key={l}><input type="checkbox" checked={visible.has(l)} onChange={()=>setVisible(v=>{const n=new Set(v);n.has(l)?n.delete(l):n.add(l);return n})}/><span className={styles[l.replace('.','')]} />{l}</label>)}<h2>Objects</h2><p>{board.footprints.length} footprints</p><p>{board.tracks.length} tracks · {board.vias.length} via</p><p>{board.issues.length} validation issue</p></aside>
  ...
  <aside className={styles.inspector}><h2>Properties</h2>{object&&'x'in object?<><strong>{'reference'in object?object.reference:object.id}</strong><label>X (mm)<input type="number" value={object.x} onChange={e=>setNum('x',+e.target.value)}/></label><label>Y (mm)<input type="number" value={object.y} onChange={e=>setNum('y',+e.target.value)}/></label>{'rotation'in object&&<label>Rotation<input type="number" value={object.rotation} onChange={e=>setNum('rotation',+e.target.value)}/></label>}<button onClick={()=>{commit(deleteSelection(board,selected));setSelected(new Set())}}>Delete selection</button></>:<p>Select a footprint, track, via, or hole. Shift-click for multi-select; drag to move.</p>}<h2>Issues</h2>{board.issues.map(i=><button className={styles.issueCard} key={i.id} title={i.message}>⚠ {i.message}</button>)}</aside>
