export function ProjectStateBadge({ state }: { state: string }) {
  return <Badge label="Project" value={state} tone={state === 'dashboard_published' ? 'green' : 'cyan'} />
}
...
export function ManufacturingReadinessBadge({ state }: { state: string }) {
  ...
  return <Badge label="Manufacturing" value={state} tone={tone} />
}

export function SourcingStatusBadge({ state }: { state: string }) {
  return <Badge label="Sourcing" value={state} tone={state === 'API_VERIFIED' ? 'green' : 'amber'} />
}

export function LicenseStatusBadge({ devMode = true }: { devMode?: boolean }) {
  return <Badge label="License" value={devMode ? 'DEV_LICENSE_MODE' : 'LICENSE_REQUIRED'} tone={devMode ? 'green' : 'amber'} />
}
