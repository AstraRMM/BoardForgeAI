...
export function WorkspaceCommandPalette() {
  ...
  const [open,setOpen] = useState(false); const [query,setQuery] = useState(''); const [projects,setProjects] = useState<Result[]>([])
  useEffect(()=>{const load=()=>setProjects(readBrowserProjects().projects.map(project=>({id:`project-${project.projectId}`,label:project.projectName,detail:`${project.projectId} · saved in this browser`,href:`/projects/${encodeURIComponent(project.projectId)}`,project:true})));load();window.addEventListener('storage',load);return()=>window.removeEventListener('storage',load)},[])
