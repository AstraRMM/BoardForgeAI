...
test('board005 replaces the unprovable PD source with a fixed source and exports exactly five KiCad files',async()=>{let invocation,exported;const driver=createPhase2cChallengeDriver({generateUsbCFixedSource:async input=>{invocation=input;return{...accepted,projectDir:'candidate'}},exportKiCadProject:async input=>{exported=input;return{files:['A.kicad_dru','A.kicad_pcb','A.kicad_prl','A.kicad_pro','A.kicad_sch']}}});const result=await driver.executeBoard(pdSource,{index:4});assert.equal(result.acceptance.accepted,true);assert.equal(invocation.liveBindings,true);assert.equal(invocation.context.index,4);assert.equal(result.acceptedBoardId,'005_USB_C_FIXED_SOURCE');assert.equal(exported.boardId,'005_USB_C_FIXED_SOURCE');assert.equal(exported.projectDir,'candidate');assert.equal(result.delivery.files.length,5)})
...
test('board005 rejected fixed-source evidence cannot export or advance checkpoint',async()=>{const root=await mkdtemp(path.join(os.tmpdir(),'bf-fixed-source-blocked-')),pilot=await authenticResult(root,'pilot'),stm=await authenticResult(root,'stm'),rp=await authenticResult(root,'rp'),sink=await authenticResult(root,'sink');let exported=false;const fake={acceptance:{status:'BOARD_ACCEPTED',accepted:true,evidenceDigest:'a'.repeat(64)},manufacturingEvidence:{status:'MANUFACTURING_REJECTED',sourceProtection:{unchanged:true},artifacts:[]}},driver=createPhase2cChallengeDriver({root,generateStm32:async()=>stm,generateRp2040:async()=>rp,generateUsbCPdSink:async()=>sink,generateUsbCFixedSource:async()=>fake,exportKiCadProject:async()=>{exported=true}});driver.executePilot=async()=>pilot;const result=await runAutonomousChallenge({manifest:{boards:[{id:'ESP32_SENSOR_HUB',slug:'esp32-sensor-hub'},stm32,rp2040,pdSink,pdSource]},checkpointPath:path.join(root,'checkpoint.json'),executePilot:driver.executePilot,executeBoard:driver.executeBoard,batchSize:4});assert.equal(result.state.nextBoardIndex,4);assert.equal(exported,false);assert.deepEqual(result.state.accepted.map(row=>row.index),[0,1,2,3])})
test('accepted fixed-source board005 advances checkpoint under replacement identity',async()=>{const root=await mkdtemp(path.join(os.tmpdir(),'bf-fixed-source-ok-')),pilot=await authenticResult(root,'pilot'),stm=await authenticResult(root,'stm'),rp=await authenticResult(root,'rp'),sink=await authenticResult(root,'sink'),source={...(await authenticResult(root,'source')),projectDir:'candidate'},driver=createPhase2cChallengeDriver({root,generateStm32:async()=>stm,generateRp2040:async()=>rp,generateUsbCPdSink:async()=>sink,generateUsbCFixedSource:async()=>source,exportKiCadProject:async()=>({files:['A.kicad_dru','A.kicad_pcb','A.kicad_prl','A.kicad_pro','A.kicad_sch']})});driver.executePilot=async()=>pilot;const result=await runAutonomousChallenge({manifest:{boards:[{id:'ESP32_SENSOR_HUB',slug:'esp32-sensor-hub'},stm32,rp2040,pdSink,pdSource]},checkpointPath:path.join(root,'checkpoint.json'),executePilot:driver.executePilot,executeBoard:driver.executeBoard,batchSize:4});assert.equal(result.status,'CHALLENGE_COMPLETE');assert.equal(result.state.nextBoardIndex,5);assert.equal(result.state.accepted.at(-1).boardId,'005_USB_C_FIXED_SOURCE')})
