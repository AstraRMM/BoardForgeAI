...
import { planSolutionAutoApply } from '../lib/solution-library/solution-auto-apply.mjs'
...
test('solution library auto-apply produces guarded lesson plan before board runs', () => {
  const plan = planSolutionAutoApply({
    boardType: 'ESC',
    netRoles: ['GATE_DRIVE'],
    drcFamilies: ['clearance'],
    constraints: ['existing-drc-debt'],
  }, { generatedAt: '2026-07-01T00:00:00.000Z' })
  assert.equal(plan.schema, 'boardforge.solution-auto-apply-plan.v1')
  assert.equal(plan.guard.doNotFakeDrcErc, true)
  assert.equal(plan.lessonsApplied.some((lesson) => lesson.id === 'esc_generated_drc_repair_001'), true)
})
