...
test('Board012 freezes exact source-backed isolation and fails closed on the unsatisfied power budget',()=>{const p=usbIsolatorProductionProposal,g=validateUsbIsolatorProductionProposal(p);assert.equal(p.bom.find(x=>x.ref==='U1').mpn,'ADUM3165BRSZ');assert.equal(p.bom.find(x=>x.ref==='U1').footprint,'Package_SO:SSOP-20_5.3x7.2mm_P0.65mm');assert.equal(p.bom.find(x=>x.ref==='U2').mpn,null);assert.equal(p.requirements.minimumIsolatedOutputMa,569);assert.equal(g.ok,false);assert.ok(g.errors.includes('usb-isolator-exact-assets-unapproved'));assert.deepEqual(g.blockedRefs,['U1','U2'])})

test('Board012 requires exact clock DFP policy both-side ESD and four bypass points in separate ground domains',()=>{const p=usbIsolatorProductionProposal;for(const ref of ['J_UP','J_DN','D_UP','D_DN','U_DFP','Y1','C_VBUS1','C_VDD1','C_VBUS2','C_VDD2'])assert.ok(p.bom.some(x=>x.ref===ref),ref);assert.notEqual(p.domains.upstream.ground,p.domains.downstream.ground);assert.deepEqual(p.domains.directCopperCrossings,[]);const broken=structuredClone(p);broken.domains.downstream.ground='GND_UP';assert.ok(validateUsbIsolatorProductionProposal(broken).errors.includes('usb-isolator-ground-domain-separation-invalid'))})
