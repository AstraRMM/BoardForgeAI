...
test('create_kicad_project preserves an explicit user BOM and resolves its local KiCad candidates', async () => {
  const workspace = await mkdtemp(path.join(tmpdir(), 'boardforge-explicit-bom-test-'))
  try {
    const components = [
      { ref: 'U1', group: 'ESP32_S3', value: 'ESP32-S3-WROOM-1' },
      { ref: 'J1', group: 'USB', value: 'USB-C receptacle' },
      { ref: 'U2', group: 'REGULATOR', value: '3V3 regulator' },
      { ref: 'J2', group: 'SENSOR_CONNECTOR', value: 'I2C sensor header' },
    ]
    const result = await executeJob({
      id: 'explicit_bom_project',
      type: 'create_kicad_project',
      allowOverwrite: true,
      input: { projectName: 'Explicit BOM Sensor', templateId: 'ESP32_S3_SENSOR', components },
    }, workspace)
    assert.equal(result.status, 'KICAD_PROJECT_CREATED_NEEDS_REVIEW')
    const state = JSON.parse(await readFile(path.join(result.projectPath, 'boardforge-project.json'), 'utf8'))
    assert.deepEqual(state.components.map((component) => component.ref), ['U1', 'J1', 'U2', 'J2'])
    assert.equal(state.components.find((component) => component.ref === 'U1').value, 'ESP32-S3-WROOM-1')
    const usbFootprint = state.components.find((component) => component.ref === 'J1').footprint
    assert.equal(typeof usbFootprint === 'string' ? usbFootprint : usbFootprint.libId, 'Connector_USB:USB_C_Receptacle_HRO_TYPE-C-31-M-12')
    const pcb = await readFile(path.join(result.projectPath, 'explicit-bom-sensor.kicad_pcb'), 'utf8')
    assert.match(pcb, /BoardForge_RF_Module_ESP32-S3-WROOM-1_generated/)
    assert.match(pcb, /USB_C_Receptacle_HRO_TYPE-C-31-M-12/)
    assert.doesNotMatch(pcb, /BoardForge_Proof/)
  } finally {
    await rm(workspace, { recursive: true, force: true })
  }
})
