...
test('custom outline generator has presets validation and seed export components', async () => {
  ...
  assert.match(editor, /boardforge\.custom-outline\.payload\.v1/)
  assert.match(editor, /Create local KiCad outline/)
  assert.match(editor, /Save browser outline draft/)
...
test('web dashboard demo data labels local artifact status and demo command', async () => {
  ...
  assert.match(dashboardPage, /DashboardLocalContent/)
  assert.match(dashboardPage, /Engineering work, grounded in recorded evidence/)
  assert.match(newBoardPage, /NewBoardIntakeWorkspace/)
