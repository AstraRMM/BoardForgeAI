...
export function productionAssetPinSchema({symbolPinMap,footprintPadMap,pinAliases={},footprintPadAliases={},unconnectedFootprintPads=[]}={}){
  ...
  const intentionallyUnconnectedPads=new Set(unconnectedFootprintPads.map(String))
  const errors=[]
  ...
  for(const pad of Object.keys(footprintPadMap)){if(intentionallyUnconnectedPads.has(pad))errors.push(`unconnected-footprint-pad-must-not-have-logical-net:${pad}`);const canonicalPad=padAliases[pad]||pad;if(!Object.keys(symbolPinMap).some(pin=>(aliases[pin]||pin)===canonicalPad))errors.push(`footprint-pad-without-symbol-pin:${pad}`);else if(footprintPadMap[pad]!==footprintPadMap[canonicalPad])errors.push(`footprint-pad-alias-logical-net-mismatch:${pad}->${canonicalPad}`)}
  return {schema:PRODUCTION_ASSET_PIN_SCHEMA,symbolPinMap:Object.freeze({...symbolPinMap}),footprintPadMap:Object.freeze({...footprintPadMap}),pinAliases:Object.freeze(aliases),footprintPadAliases:Object.freeze(padAliases),unconnectedFootprintPads:Object.freeze([...intentionallyUnconnectedPads]),valid:errors.length===0,errors}
}
