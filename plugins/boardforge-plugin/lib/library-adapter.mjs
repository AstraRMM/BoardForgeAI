...
const symbolFamilyAliases = {
  ESP32_S3: ['esp32'],
  REGULATOR: ['regulator'],
  IMU: ['sensor_motion', 'inertial', 'mems'],
  BAROMETER: ['sensor_pressure', 'bmp280', 'barometer'],
  ETHERNET_PHY: ['ethernet', 'lan8720', 'phy'],
  POE_FRONT_END: ['poe', 'power_management'],
  RJ45: ['rj45', '8p8c'],
  CURRENT_SENSOR: ['ina180', 'current'],
  CAN_TRANSCEIVER: ['can'],
  RS485_TRANSCEIVER: ['rs485'],
  TVS: ['tvs'],
}
...
// KiCad's installed libraries are immutable for the duration of a BoardForge
// process. Reuse the parsed index across fixture jobs so a regression run does
// not repeatedly deserialize the same large manifest for every board.
...
async function loadOrBuildManifest(workspace, input = {}) {
  ...
  if (!input.refresh && manifestPromiseCache.has(cacheKey)) return manifestPromiseCache.get(cacheKey)
  const load = loadOrBuildManifestUncached(workspace, input, manifestPath)
  if (!input.refresh) manifestPromiseCache.set(cacheKey, load)
  try {
    return await load
  } catch (error) {
    manifestPromiseCache.delete(cacheKey)
    throw error
  }
}

async function loadOrBuildManifestUncached(workspace, input, manifestPath) {
...
function resolveSingleComponent(component, manifest, input = {}) {
  ...
  const matchingFootprints = rankAssets(manifest.footprints, terms).filter((asset) => assetMatchesComponentFamily(asset, component, input, 'footprint'))
  const matchingSymbols = rankAssets(manifest.symbols, terms).filter((asset) => assetMatchesComponentFamily(asset, component, input, 'symbol'))
  const footprint = forced || firstByLibId(manifest.footprints, preferred.footprints) || matchingFootprints[0] || null
  const symbol = forcedSymbolId ? manifest.symbols.find((asset) => asset.libId === forcedSymbolId) : firstByLibId(manifest.symbols, preferred.symbols) || matchingSymbols[0] || null
...
function assetMatchesComponentFamily(asset, component, input = {}, kind = 'footprint') {
  const aliases = kind === 'symbol' && symbolFamilyAliases[component.group]
    ? symbolFamilyAliases[component.group]
    : defaultAliases[component.group] || []
  // Unknown groups are only allowed to use fuzzy ranking when a caller has
  // explicitly requested it. This keeps an arbitrary search result from being
  // promoted into an apparently valid engineering binding.
  if (!aliases.length) return input.allowHeuristicFallback === true || input.strict !== true
  const haystack = normalizeText([asset.libId, asset.name, asset.library, asset.description, asset.tags].filter(Boolean).join(' '))
