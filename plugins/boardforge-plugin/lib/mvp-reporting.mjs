...
export function scoreMvpReadiness90(fixtures = [], evidence = {}) {
  ...
  const poe = fixtures.find((fixture) => ['poe_sensor_electrical_cached', 'poe_ethernet_sensor'].includes(fixture.id))
  ...
  setScore(scores, 'Sandboxed imported-board repair', importedRepairProofs.length >= 1 ? 9 : 4, importedRepairProofs.length >= 1 ? `${importedRepairProofs.length} imported-board sandbox repair proof(s) kept source hashes untouched while repairing the copy to clean manufacturing output.` : 'No imported-board sandbox repair proof is available yet.', 'Run repair against more imported synthetic/customer-safe projects.')
  ...
  setScore(scores, 'Existing project scan', importedRepairProofs.length >= 1 ? 9 : existingScan?.scan?.status ? 8 : 5, importedRepairProofs.length >= 1 ? 'Sandboxed imported-board proof copied a KiCad project, hash-guarded the source, scanned/repaired only the sandbox, and exported clean output.' : existingScan?.scan?.summary || 'Existing project scan fixture not proven.', 'Repeat sandbox import proof on more external/synthetic boards.')
