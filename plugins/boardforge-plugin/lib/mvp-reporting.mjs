...
export function scoreMvpReadiness90(fixtures = [], evidence = {}) {
  ...
  setScore(scores, 'PoE isolation reasoning', poe?.id === 'poe_sensor_depth_rev_b_cached' && poe?.drc?.errors === 0 ? 9 : poe?.expectedFailure && poe?.honestFailure ? 7 : poe?.drc?.errors === 0 ? 8 : 5, poe ? `PoE DRC errors: ${poe.drc?.errors ?? 'n/a'} with explicit compliance/isolation honesty evidence.` : 'PoE fixture missing.', 'Implement real PoE isolation/magnetics/RJ45 model or keep exact irreducible blocker.')
