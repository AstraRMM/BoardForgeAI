...
async function createKiCadProject(job, workspace, profile) {
  ...
  const requirementsPlan = (job.input?.prompt || job.input?.notes || job.input?.components?.length || job.input?.interfaces?.length)
    ...
    : null
  // A user-provided BOM is authoritative. Templates and requirement planning
  // may fill gaps, but must never silently replace the parts a user asked
  // BoardForge to validate and place.
  const explicitComponents = Array.isArray(job.input?.components) && job.input.components.length
    ? job.input.components
    : null
  const seedComponents = explicitComponents || (requirementsPlan?.components?.length ? requirementsPlan.components : generateTemplateComponents(board, job.input?.templateId))
