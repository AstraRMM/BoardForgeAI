...
export function productionAssetBindings(report) {
  return {status:report?.status,manufacturingAllowed:report?.manufacturingAllowed===true,components:(report?.components||[]).map(row=>{const mpn=row.canonicalBinding?.binding?.manufacturerPartNumber||row.exactMpnRequirement,rawPinMap=row.canonicalBinding?.binding?.pinMap||row.pinMap,pinMap=Object.fromEntries(Object.entries(rawPinMap||{}).filter(([,net])=>typeof net==='string'&&net.trim())),asset=approvedAssetFor(mpn),aliases=asset?.pinAliases||{},hasGroupedPads=Object.keys(asset?.footprintPadAliases||{}).length>0,padMap=hasGroupedPads?{...pinMap}:Object.fromEntries(Object.entries(pinMap).map(([pin,net])=>[aliases[pin]||pin,net])),expectedUnconnectedSymbolPins=Object.entries(rawPinMap||{}).filter(([,net])=>net==null).map(([pin])=>String(pin)),expectedUnconnectedFootprintPads=expectedUnconnectedSymbolPins.map(pin=>hasGroupedPads?pin:(aliases[pin]||pin));const tpsSourceShort=mpn==='TPS25750DRJKR'&&row.ref==='U2'&&['23','24','25','32','33'].every(pin=>pinMap?.[pin]==='VBUS');if(tpsSourceShort)padMap['23']='VBUS_IN';return{ref:row.ref,mpn,exactMpnVerified:row.canonicalBinding?.status==='BOUND',symbol:row.canonicalBinding?.projections?.schematic?.symbol?.libId||row.symbol,footprint:row.canonicalBinding?.projections?.schematic?.footprint||row.footprint,pinMapVerified:row.canonicalBinding?.status==='BOUND',pinMap,symbolPinMap:pinMap,footprintPadMap:padMap,expectedUnconnectedSymbolPins,expectedUnconnectedFootprintPads,pinAliases:aliases,physicalFootprintPadMap:asset?.footprintPadMap||null,physicalPadAliases:asset?.footprintPadAliases||null,physicalNetEquivalencePolicy:tpsSourceShort?TPS25750_SOURCE_VBUS_EQUIVALENCE:null,bindingId:row.canonicalBinding?.binding?.bindingId||null}})}
}
...
const acceptanceRun=(run,reportPath,executedAt)=>({tool:'kicad-cli',exitCode:run.exitCode,errors:run.issueCounts?.errors||0,violations:issueTotal(run),reportPath,executedAt,ignoredChecks:Array.isArray(run.report?.ignored_checks)?run.report.ignored_checks:[]})
