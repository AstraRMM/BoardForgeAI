...
export function productionAssetBindings(report) {
  return {status:report?.status,manufacturingAllowed:report?.manufacturingAllowed===true,components:(report?.components||[]).map(row=>{const mpn=row.canonicalBinding?.binding?.manufacturerPartNumber||row.exactMpnRequirement,pinMap=row.canonicalBinding?.binding?.pinMap||row.pinMap,asset=approvedAssetFor(mpn),aliases=asset?.pinAliases||{},padMap=Object.fromEntries(Object.entries(pinMap||{}).map(([pin,net])=>[aliases[pin]||pin,net]));const tpsSourceShort=mpn==='TPS25750DRJKR'&&row.ref==='U2'&&['23','24','25','32','33'].every(pin=>pinMap?.[pin]==='VBUS');if(tpsSourceShort)padMap['23']='VBUS_IN';return{ref:row.ref,mpn,exactMpnVerified:row.canonicalBinding?.status==='BOUND',symbol:row.canonicalBinding?.projections?.schematic?.symbol?.libId||row.symbol,footprint:row.canonicalBinding?.projections?.schematic?.footprint||row.footprint,pinMapVerified:row.canonicalBinding?.status==='BOUND',pinMap,symbolPinMap:pinMap,footprintPadMap:padMap,pinAliases:aliases,physicalNetEquivalencePolicy:tpsSourceShort?TPS25750_SOURCE_VBUS_EQUIVALENCE:null,bindingId:row.canonicalBinding?.binding?.bindingId||null}})}
}
