export const BOARD006_ISOLATION_REMEDIATION=Object.freeze({
 ...
 /** The manufacturer drawing confirms the package envelope and pinout below.
  * It does not publish a PCB land pattern, pad diameter, or drill diameter.
  * Keep this evidence deliberately separate from an approved KiCad footprint:
  * guessing those fabrication dimensions would make a non-production asset. */
 isolatedPowerMechanicalEvidence:Object.freeze({
  source:'https://www.tracopower.com/products/thi2m.pdf',package:'DIP-16',view:'bottom',
  bodyLengthMm:23.8,bodyWidthMm:13.4,bodyHeightMm:8.6,pinPitchMm:2.54,rowPitchMm:10.16,
  electricalPins:Object.freeze([1,7,8,9,10,16]),
  geometryStatus:'SOURCE_BACKED_PACKAGE_ENVELOPE_ONLY',
  fabricationLandPatternStatus:'UNRESOLVED_DO_NOT_REGISTER_AS_PRODUCTION_ASSET',
 }),
...
export function validateBoard006IsolationRemediation(evidence={}){
 ...
 const mechanical=evidence.isolatedPowerMechanicalEvidence||{},expectedMechanical=c.isolatedPowerMechanicalEvidence
 if(mechanical.primarySourceUrl!==expectedMechanical.source||mechanical.primarySourceVerified!==true||mechanical.package!==expectedMechanical.package||mechanical.view!==expectedMechanical.view)errors.push('replacement-isolated-power-mechanical-source-unverified')
 for(const key of['bodyLengthMm','bodyWidthMm','bodyHeightMm','pinPitchMm','rowPitchMm'])if(mechanical[key]!==expectedMechanical[key])errors.push(`replacement-isolated-power-mechanical-${key}-mismatch`)
 if(mechanical.fabricationLandPatternStatus!==expectedMechanical.fabricationLandPatternStatus)errors.push('replacement-isolated-power-fabrication-status-mismatch')
