...
export async function enrichComponents({ workspace, components, input = {} }) {
  // A generic word match against KiCad's huge library can produce a real but
  // unrelated footprint (for example, a battery holder for an unspecified
  // wearable MCU). Only auto-resolve catalog-backed or explicitly identified
  // parts unless a caller deliberately opts into heuristic search.
  const assetEligible = components.filter((component) => {
    const explicit = component.symbol || component.footprint || component.mpn || component.lcsc
    return explicit || Boolean(bestCatalogMatch(component)) || input.allowHeuristicFallback === true
  })
  const assets = await resolveComponentAssets({ workspace, input: { ...input, components: assetEligible } })
  return Promise.all(components.map(async (component) => {
    const base = bestCatalogMatch(component)
    const resolved = assets.components?.find((item) => item.ref === component.ref)
    const mayResolve = Boolean(base || component.symbol || component.footprint || component.mpn || component.lcsc || input.allowHeuristicFallback === true)
    const symbol = mayResolve ? preferRichAsset(component.symbol, resolved?.symbol) : null
    const footprint = mayResolve ? preferRichAsset(component.footprint, resolved?.footprint) : null
    const model3d = mayResolve ? preferRichAsset(component.model3d, resolved?.model3d) : null
    ...
    return {
      ...candidate,
      assetStatus,
      confidence: mayResolve ? (resolved?.confidence || 'needs_review') : 'unresolved_no_catalog_or_explicit_asset',
