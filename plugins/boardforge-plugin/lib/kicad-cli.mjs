...
async function runReportCommand({ kicadCliPath, args, outputFile, reportKind }) {
  ...
  // kicad-cli can finish its process before the JSON writer has flushed the
  // final report, especially when --save-board also rewrites the PCB. Read a
  // stable JSON snapshot so BoardForge never records a stale DRC count.
  let report = await waitForStableJsonReport(outputFile)
  if (!report) {
...
async function waitForStableJsonReport(file, attempts = 12, delayMs = 125) {
  let previous = null
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    const report = await readJsonIfExists(file)
    if (report) {
      const snapshot = JSON.stringify(report)
      if (snapshot === previous) return report
      previous = snapshot
    }
